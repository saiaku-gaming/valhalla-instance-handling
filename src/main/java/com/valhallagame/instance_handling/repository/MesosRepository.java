...
public interface MesosRepository extends JpaRepository<MesosFramework, String> {
	public MesosFramework findById(String id);
	
	@Query(value = "SELECT * FROM mesos_framework ORDER BY ts DESC LIMIT 1", nativeQuery = true)
	public MesosFramework getLatestMesosFramework();
	...
	public MesosFramework getLatestValidMesosFramework(@Param("failoverTimeout") double failoverTimeout);
}
